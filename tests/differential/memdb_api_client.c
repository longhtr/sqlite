...
int main(void){
  ...
  int partial_and_index_rc=sqlite3_exec(clone,"CREATE UNIQUE INDEX partial_and_index ON partial_and_data(value) WHERE lower_bound >= 0 AND upper_bound < 10",0,0,0);
  int partial_and_outside_rc=sqlite3_exec(clone,"INSERT INTO partial_and_data VALUES(1,20,-1,5)",0,0,0);
  int partial_and_inside_rc=sqlite3_exec(clone,"INSERT INTO partial_and_data VALUES(2,20,0,5)",0,0,0);
  int partial_and_duplicate_rc=sqlite3_exec(clone,"INSERT INTO partial_and_data VALUES(3,20,0,9)",0,0,0);
  int partial_and_enter_conflict_rc=sqlite3_exec(clone,"UPDATE partial_and_data SET lower_bound=0 WHERE id=1",0,0,0);
  int partial_and_delete_rc=sqlite3_exec(clone,"DELETE FROM partial_and_data WHERE id=2",0,0,0);
  int partial_and_enter_rc=sqlite3_exec(clone,"UPDATE partial_and_data SET lower_bound=0 WHERE id=1",0,0,0);
