...
pub const Pager = struct {
    ...
    pub fn journalFile(self: *Pager) ?*vfs.sqlite3_file {
        if (self.wal_state) |*state| {
            if (state.walFile()) |file| return file;
        }
...
test "journal file accessor selects rollback journal and open WAL handles" {
    ...
    try std.testing.expect(!wal.Wal.usesHeapIndex(null));
    try std.testing.expect(!wal.Wal.usesHeapIndex(&pager.wal_state.?));
    try std.testing.expect(pager.wal_state.?.walFile() == null);
    ...
    try std.testing.expect(pager.journalFile() == pager.wal_state.?.walFile());
