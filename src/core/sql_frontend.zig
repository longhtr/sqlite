...
fn isNumericDeclaredType(declared_type: []const u8) bool {
    if (declared_type.len == 0) return false;
    if (containsAsciiIgnoreCase(declared_type, "INT")) return true;
    if (containsAsciiIgnoreCase(declared_type, "CHAR") or containsAsciiIgnoreCase(declared_type, "CLOB") or containsAsciiIgnoreCase(declared_type, "TEXT") or containsAsciiIgnoreCase(declared_type, "BLOB")) return false;
    return true;
}
...
fn resolveIndexPredicateTermInner(token_list: []const Token, columns: []const ResolvedColumn, position: *usize) error{Syntax}!btree.IndexPredicateTerm {
    ...
    if (position.* < token_list.len and (token_list[position.*].typ == tokens.tk_float or ((token_list[position.*].typ == tokens.tk_minus or token_list[position.*].typ == tokens.tk_plus) and position.* + 1 < token_list.len and token_list[position.* + 1].typ == tokens.tk_float))) {
        ...
        const column_index = selected orelse return error.Syntax;
        if (!columns[column_index].integer_primary_key and !isNumericDeclaredType(columns[column_index].declared_type)) return error.Syntax;
        position.* += 1;
...
fn programActionCallback(context: ?*anyopaque, arguments: []vdbe.Mem, output: *vdbe.Mem, allocator: std.mem.Allocator) ResultCode {
    ...
    return switch (owner.action orelse return .misuse) {
        ...
        .alter_rename_table => |action| blk: {
            ...
            const renamed = action.database.renameSchemaTable(action.old_name, action.new_name, action.sql);
            if (renamed != .ok) break :blk renamed;
            const committed = action.database.commitStatementBatch();
            batch_active = false;
            break :blk action.connection.afterWrite(committed, null, action.schema_name, action.new_name, 0);
        },
...
fn compileAlterTable(connection: *Connection, source: [:0]u8, token_list: []const Token, consumed: usize) CompileOutcome {
    ...
    if (position + 2 < token_list.len and token_list[position].typ == tokens.tk_rename and token_list[position + 1].typ == tokens.tk_to and schemaIdentifierToken(token_list[position + 2]) and position + 3 == token_list.len) {
        const new_name = token_list[position + 2].text;
        const target_database = locateDatabase(connection, schema_name);
        if (target_database.result != .ok) {
            allocator.free(source);
            return .{ .result = if (target_database.result == .not_found) .error_ else target_database.result, .consumed = consumed };
        }
        const target_exists = target_database.database.?.schemaTableExists(new_name);
        if (target_exists.result != .ok or target_exists.found) {
            allocator.free(source);
            return .{ .result = if (target_exists.result == .ok) .error_ else target_exists.result, .consumed = consumed };
        }
        const located = locateTableWithDatabase(connection, table_name, schema_name);
        if (located.result != .ok) {
            allocator.free(source);
        ...
        var table = located.table.?;
        defer table.deinit();
        const updated_sql = schema_program_runtime.renameTableSql(allocator, table.sql, table_name, new_name, false) catch |err| {
            allocator.free(source);
            return .{ .result = if (err == error.OutOfMemory) .no_memory else .error_, .consumed = consumed };
        };
        const owner = allocator.create(Owner) catch {
...
test "public open close error and deferred statement lifecycle" {
    ...
    try std.testing.expectEqual(@as(c_int, 0), sqlite3_txn_state(database, null));
    ...
    try std.testing.expectEqual(@as(c_int, -1), sqlite3_db_readonly(database, "temp"));
    try std.testing.expectEqual(@as(c_int, -1), sqlite3_db_readonly(database, "missing"));
    var prepared: ?*statement.sqlite3_stmt = null;
