...
fn resolveColumns(allocator: std.mem.Allocator, sql: []const u8) !struct { source: [:0]u8, tokens: []Token, columns: []ResolvedColumn } {
    ...
    while (position < parsed.tokens.len and parsed.tokens[position].typ != tokens.tk_rp) {
        ...
        } else if (schema_is_index and position + 3 < parsed.tokens.len and parsed.tokens[position].typ == tokens.tk_id and (std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "abs") or std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "sign") or std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "typeof") or std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "octet_length") or std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "length") or std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "unicode")) and parsed.tokens[position + 1].typ == tokens.tk_lp and parsed.tokens[position + 2].typ == tokens.tk_id and parsed.tokens[position + 3].typ == tokens.tk_rp) {
            scan_expression = true;
            index_transform = if (std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "sign")) .numeric_sign else if (std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "typeof")) .storage_type else if (std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "octet_length")) .octet_length else if (std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "length")) .text_length else if (std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "unicode")) .unicode_value else .numeric_abs;
...
fn compileIndexSchema(connection: *Connection, source: [:0]u8, token_list: []const Token, consumed: usize) CompileOutcome {
    ...
    while (true) {
        ...
        } else if (position + 3 < token_list.len and token_list[position].typ == tokens.tk_id and (std.ascii.eqlIgnoreCase(token_list[position].text, "abs") or std.ascii.eqlIgnoreCase(token_list[position].text, "sign") or std.ascii.eqlIgnoreCase(token_list[position].text, "typeof") or std.ascii.eqlIgnoreCase(token_list[position].text, "octet_length") or std.ascii.eqlIgnoreCase(token_list[position].text, "length") or std.ascii.eqlIgnoreCase(token_list[position].text, "unicode")) and token_list[position + 1].typ == tokens.tk_lp and token_list[position + 2].typ == tokens.tk_id and token_list[position + 3].typ == tokens.tk_rp) {
            transform = if (std.ascii.eqlIgnoreCase(token_list[position].text, "sign")) .numeric_sign else if (std.ascii.eqlIgnoreCase(token_list[position].text, "typeof")) .storage_type else if (std.ascii.eqlIgnoreCase(token_list[position].text, "octet_length")) .octet_length else if (std.ascii.eqlIgnoreCase(token_list[position].text, "length")) .text_length else if (std.ascii.eqlIgnoreCase(token_list[position].text, "unicode")) .unicode_value else .numeric_abs;
