...
fn resolveColumns(allocator: std.mem.Allocator, sql: []const u8) !struct { source: [:0]u8, tokens: []Token, columns: []ResolvedColumn } {
    ...
    while (position < parsed.tokens.len and parsed.tokens[position].typ != tokens.tk_rp) {
        ...
        } else if (schema_is_index and position + 3 < parsed.tokens.len and parsed.tokens[position].typ == tokens.tk_id and (std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "abs") or std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "sign") or std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "round") or std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "typeof") or std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "octet_length") or std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "length") or std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "unicode") or std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "likely") or std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "unlikely")) and parsed.tokens[position + 1].typ == tokens.tk_lp and parsed.tokens[position + 2].typ == tokens.tk_id and parsed.tokens[position + 3].typ == tokens.tk_rp) {
            scan_expression = true;
            index_transform = if (std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "sign")) .numeric_sign else if (std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "round")) .numeric_round else if (std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "typeof")) .storage_type else if (std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "octet_length")) .octet_length else if (std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "length")) .text_length else if (std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "unicode")) .unicode_value else if (std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "likely") or std.ascii.eqlIgnoreCase(parsed.tokens[position].text, "unlikely")) .identity else .numeric_abs;
            name = parsed.tokens[position + 2].text;
        } else {
...
fn compileIndexSchema(connection: *Connection, source: [:0]u8, token_list: []const Token, consumed: usize) CompileOutcome {
    ...
    while (true) {
        ...
        if (resolveIfnullIndexExpression(token_list, position)) |ifnull_expression| {
            ...
            position += 6;
        } else if (position + 3 < token_list.len and token_list[position].typ == tokens.tk_id and (std.ascii.eqlIgnoreCase(token_list[position].text, "abs") or std.ascii.eqlIgnoreCase(token_list[position].text, "sign") or std.ascii.eqlIgnoreCase(token_list[position].text, "round") or std.ascii.eqlIgnoreCase(token_list[position].text, "typeof") or std.ascii.eqlIgnoreCase(token_list[position].text, "octet_length") or std.ascii.eqlIgnoreCase(token_list[position].text, "length") or std.ascii.eqlIgnoreCase(token_list[position].text, "unicode") or std.ascii.eqlIgnoreCase(token_list[position].text, "likely") or std.ascii.eqlIgnoreCase(token_list[position].text, "unlikely")) and token_list[position + 1].typ == tokens.tk_lp and token_list[position + 2].typ == tokens.tk_id and token_list[position + 3].typ == tokens.tk_rp) {
            transform = if (std.ascii.eqlIgnoreCase(token_list[position].text, "sign")) .numeric_sign else if (std.ascii.eqlIgnoreCase(token_list[position].text, "round")) .numeric_round else if (std.ascii.eqlIgnoreCase(token_list[position].text, "typeof")) .storage_type else if (std.ascii.eqlIgnoreCase(token_list[position].text, "octet_length")) .octet_length else if (std.ascii.eqlIgnoreCase(token_list[position].text, "length")) .text_length else if (std.ascii.eqlIgnoreCase(token_list[position].text, "unicode")) .unicode_value else if (std.ascii.eqlIgnoreCase(token_list[position].text, "likely") or std.ascii.eqlIgnoreCase(token_list[position].text, "unlikely")) .identity else .numeric_abs;
            column_name = token_list[position + 2].text;
            position += 4;
        } else {
    ...
    for (column_names.items, 0..) |column_name, selected_position| {
        ...
        const numeric_transform = switch (specified_transforms.items[selected_position]) {
            .identity, .storage_type, .octet_length, .text_length, .unicode_value, .is_null, .is_not_null, .null_coalesce_integer, .null_if_integer => false,
