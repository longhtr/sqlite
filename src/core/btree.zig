...
pub fn transformIndexValue(transform: IndexTransform, value: Value) Value {
    switch (transform) {
        ...
        .numeric_ceil, .numeric_floor, .numeric_trunc => return switch (value) {
            .null_ => .null_,
            .integer => value,
            .real => |real| .{ .real = switch (transform) {
                .numeric_ceil => @ceil(real),
                .numeric_floor => @floor(real),
                .numeric_trunc => @trunc(real),
                else => unreachable,
            } },
            .text, .blob => .null_,
        },
    ...
    return switch (transform) {
        ...
        .numeric_sign => switch (numeric) {
            ...
            .real => |real| .{ .integer = if (real < 0) -1 else if (real > 0) 1 else 0 },
            .text, .blob => unreachable,
        },
...
pub fn indexPredicateMatches(predicate: IndexPredicateTerm, value: Value) bool {
    return switch (predicate.operation) {
        ...
        .integer_is, .integer_is_not => {
            const matches = switch (value) {
                .integer => |integer| integer == predicate.comparison_value,
                .real => |real| real == @as(f64, @floatFromInt(predicate.comparison_value)),
                else => false,
            };
            return if (predicate.operation == .integer_is) matches else !matches;
        },
